...
fn parse_args() -> Cli {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "-V" | "--version" => {
                println!("math2img-tectonic {}", env!("CARGO_PKG_VERSION"));
                process::exit(0);
            }
            other => {
                if input.is_none() {
...
fn print_help() {
    eprintln!(
        "math2img-tectonic - Extract math equations from LaTeX/Markdown and render as PNG (Tectonic backend)

USAGE:
    math2img-tectonic -i <INPUT> [OPTIONS]
